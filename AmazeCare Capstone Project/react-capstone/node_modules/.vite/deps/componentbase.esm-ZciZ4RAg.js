...
function mask(el, options) {
	...
	var caret = function caret(first, last) {
		...
		if (typeof first === "number") {
			...
			if (el.setSelectionRange) el.setSelectionRange(begin, end);
	...
	var isCompleted = function isCompleted() {
		for (var i = firstNonMaskPos; i <= lastRequiredNonMaskPos; i++) if (tests[i] && buffer[i] === getPlaceholder(i)) return false;
		return true;
	};
	var getPlaceholder = function getPlaceholder(i) {
		if (i < options.slotChar.length) return options.slotChar.charAt(i);
	...
	var handleAndroidInput = function handleAndroidInput(e) {
		...
		if (oldVal && oldVal.length && oldVal.length > curVal.length) {
			...
			if (pos.begin === 0) while (pos.begin < firstNonMaskPos && !tests[pos.begin]) pos.begin++;
		...
		if (options.onComplete && isCompleted()) options.onComplete({
			originalEvent: e,
			value: getValue()
		});
	};
	...
	var onKeyPress = function onKeyPress(e) {
		...
		var next;
		var completed;
		if (e.ctrlKey || e.altKey || e.metaKey || k < 32) return;
		else if (k && k !== 13) {
			...
			if (p < len) {
				...
				if (tests[p].test(c)) {
					...
					writeBuffer();
					next = seekNext(p);
					if (DomHandler.isAndroid()) setTimeout(function proxy() {
						caret(next);
					}, 0);
					else caret(next);
					if (pos.begin <= lastRequiredNonMaskPos) completed = isCompleted();
				}
			}
			e.preventDefault();
		}
		updateModel(e);
		if (options.onComplete && completed) options.onComplete({
			originalEvent: e,
			value: getValue()
		});
	};
	var clearBuffer = function clearBuffer(start, end) {
		var i;
		for (i = start; i < end && i < len; i++) if (tests[i]) buffer[i] = getPlaceholder(i);
	};
	...
	var checkVal = function checkVal(allow) {
		...
		else if (lastMatch + 1 < partialPosition) if (options.autoClear || buffer.join("") === defaultBuffer) {
	...
	var onFocus = function onFocus(e) {
		...
		caretTimeoutId = setTimeout(function() {
			...
			writeBuffer();
			if (pos === options.mask.replace("?", "").length) caret(0, pos);
			else caret(pos);
		}, 100);
	...
	var getUnmaskedValue = function getUnmaskedValue() {
		...
		for (var i = 0; i < buffer.length; i++) {
			var c = buffer[i];
			if (tests[i] && c !== getPlaceholder(i)) unmaskedBuffer.push(c);
		}
	...
	var updateModel = function updateModel(e) {
		if (options.onChange) {
			...
			options.onChange({
				...
				target: { value: defaultBuffer !== val ? val : "" }
			});
		}
	};
	...
	var init = function init() {
		...
		for (var _i = 0; _i < maskTokens.length; _i++) {
			...
			if (_c !== "?") if (defs[_c]) buffer.push(getPlaceholder(_i));
			else buffer.push(_c);
		}
...
//#endregion
export { UniqueComponentId as C, mask as E, ObjectUtils as S, classNames as T, localeOption as _, useGlobalOnEscapeKey as a, EventBus as b, useOverlayListener as c, useResizeListener as d, useStyle as f, PrimeReactContext as g, PrimeReact as h, useDisplayOrder as i, useOverlayScrollListener as l, useUpdateEffect as m, useHandleStyle as n, useMergeProps as o, useUnmountEffect as p, ESC_KEY_HANDLING_PRIORITIES as r, useMountEffect as s, ComponentBase as t, usePrevious as u, localeOptions as v, ZIndexUtils as w, IconUtils as x, DomHandler as y };

//# sourceMappingURL=componentbase.esm-ZciZ4RAg.js.map